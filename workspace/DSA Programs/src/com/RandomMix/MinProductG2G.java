...
public class MinProductG2G {
	...
	public static void main(String[] args) {
		// TODO Auto-generated method stub
		 int a[] = { 2, 3, 4, 5, 4 };
		    ...
		    int num = (int) (Math.log(4)/Math.log(2));
	...
	static int minproduct(int a[], int b[], int n, int k)
	{
	    ...
	    for (int i = 0; i < n; i++) {
	        ...
	        // we must increase value of a[i] to minimize result.
	        if (pro < 0 && b[i] < 0)
	            temp = (a[i] + 2 * k) * b[i];

	        // If both product and a[i] are negative,
	        // we must decrease value of a[i] to minimize result.
	        else if (pro < 0 && a[i] < 0)
	            temp = (a[i] - 2 * k) * b[i];

	        // Similar to above two cases for positive product.
	        else if (pro > 0 && a[i] < 0)
	            temp = (a[i] + 2 * k);    // statement 1
	        else if (pro > 0 && a[i] > 0)
	            temp = (a[i] - 2 * k);    // statement 2
